...
縦積みをやめて 1 か所に重なり、トランプの手札のように広がる。

- 実装は `src/css/base.css` の 9 節。CSS だけで、JS もブロックも増えない。
- グリッドを 1 セル（`grid-column: 1; grid-row: 1`）にして重ね、`transform-style: preserve-3d`
  と各カラムの `translateZ()` で 3D の重なり争いを避ける。原作 pokemon-cards-css のデモと同じ考え方。
- **扇のずらしはカラムに、傾きは `.holo__card` に**かかるので、2 つの transform は競合しない。
- **触っているカードの transform は動かさない。** 前に出すのは `z-index` だけ。
  いちど「触ったら起こす」を入れたら、カードがポインタの下から逃げて
  離れる → 戻る → また触れる の往復になり、激しくちらついた（2026-09-12 に修正）。
  ホバーで位置が変わる要素は、その要素自身のホバー判定を壊す。
- 手前に出るのは `.holo__card.is-interacting` / `.is-lifted` を含むカラム（`:has()` で拾う）。
  同時にそのカラムの `p` を表示し、他は `opacity: 0` のまま場所だけ残す（出入りで行送りが動かない）。
  どれも触っていないときは、いちばん手前のカードのぶんを出しておく。
- カラムが 3 でも 4 でもないときは `:has()` に当たらないので WordPress 既定の縦積みのまま。
  23 種一覧のようにファミリーごとで枚数が違う並びには付けない。
- **触っていないカードは外へ逃がす。** どれかに触っている間、触っていないカラムだけ
  さらに外へ開いて（3 枚 ±35 度 9 度 / 4 枚 ±47%・±17%）、後ろのカードが見える面積を増やす。
  動かすのは触っていないカードだけなので、上のちらつきは起きない。
- **`click: lift` と組み合わせて使う。** タップしたカードが `.is-lifted` で前に固定され、
  view/index.js の `unliftAll()` が他を戻すので、手札を 1 枚ずつ見ていける
  （外側のタップか Esc で戻る）。扇だけだと後ろのカードが見えないので、この組み合わせが前提。
- 数値: 3 枚はカラム幅 52%・左右 ±30%・回転 ∓7 度で中央が最前面（開いたとき ±35%・∓9 度）。
  4 枚はカラム幅 46%・±42% と ±14%・回転 ∓9.5 度と ∓3.2 度で、内側 2 枚が手前（左から 3 番目が最前面。
  開いたとき ±47% と ±17%・∓11 度と ∓4 度）。
  どちらもコンテナに `padding-block: 14px`。390px 幅で、開いた状態でも左右に 8px 残る。
