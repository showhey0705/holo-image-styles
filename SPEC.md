...
## 付録 H. `.holo-fan` — スマートフォンで 3〜4 枚を扇状に重ねる（2026-09-12）

3 枚または 4 枚並べたカラムブロックの「追加 CSS クラス」に `holo-fan` と書くと、781px 以下で
縦積みをやめて 1 か所に重なり、トランプの手札のように広がる。あわせて画像の
**「クリック時の動作」を「前に出す」**にしておくのが前提（タップしたカードが `.is-lifted` で
前に固定され、`view/index.js` の `unliftAll()` が他を戻すので 1 枚ずつ見ていける）。

- 実装は `src/css/base.css` の 9 節。CSS だけで、JS もブロックも増えない。
- グリッドを 1 セル（`grid-column: 1; grid-row: 1`）にして重ねる。扇のずらしはカラム、
  傾きは中の `.holo__card` なので、2 つの transform は競合しない。
- 位置は `--fan-x` / `--fan-r` に持たせ、開いたときだけ `--fan-x-open` / `--fan-r-open` に
  差し替える。「開く」の切り替えが枚数ぶんではなく 1 組で済む。
- 前に出ていないカードだけ外へ開く（3 枚 ±35%・∓9 度 / 4 枚 ±47%・±17%）。
- 数値: 3 枚はカラム幅 52%・±30%・∓7 度で中央が最前面。4 枚は 46%・±42% と ±14%・
  ∓9.5 度と ∓3.2 度で内側 2 枚が手前（左から 3 番目が最前面）。コンテナに `padding-block: 14px`。
  390px 幅で、開いた状態でも左右に 8px 残る。

### つまずいた 3 点（同じ罠を踏まないように）

1. **前に出ているカードの transform は動かさない。** 触ったカードを「起こす」ために transform を
   書き換えたら、カードがポインタの下から逃げて 離れる → 戻る → また触れる の往復になり、
   激しくちらついた。ホバーで位置が変わる要素は、その要素自身のホバー判定を壊す。
   動かすのは触っていないカードだけ。
2. **`is-lifted` を `is-interacting` より強い z-index にする。** 同じ値だと DOM 順で決まり、
   後ろのカードをタップしても前に出ない。上書き側のセレクタは枚数の判定まで含めて、
   nth-child の指定より詳細度を高く保つ（`:where()` で下げると今度は core の CSS に負ける）。
3. **親に `transform-style: preserve-3d` を付けない。** 3D の文脈に入ると前後が z-index ではなく
   Z 座標で決まり、z-index をいくら上げてもタップしたカードが前に出ない
   （「タップしても拡大するだけ」の原因はこれ）。カード自身の傾きは `.holo__card` の
   `perspective()` が持つので、親に 3D は要らない。原作 pokemon-cards-css は
   `preserve-3d` ＋ `translateZ()` で順番を作っているが、こちらは z-index で作る。

`.holo-fan` の中の `p` は `position: relative; z-index: 1`。カードは transform を持つため
そのままだと下の文字より上に描かれ、前に出して拡大したカードが文字に被る。
